...
import {
  ...
  FormControlLabel, Link, Stack, Fade, Collapse,
...
export interface AuthModalProps {
  ...
  onSuccess?: (view: "signin" | "signup", data: LoginPayload | RegisterPayload) => void;
  loading?: boolean;
  error?: string | null;
}

// ── Field helpers ─────────────────────────────────────────────────────────────
interface FieldState { value: string; error: string; }
const emptyField = (): FieldState => ({ value: "", error: "" });

function validate(view: AuthView, fields: Record<string, FieldState>): Record<string, string> {
  const errors: Record<string, string> = {};

  // Username — both signin and signup
  if (view !== "forgot") {
    if (!fields.username?.value.trim())
      errors.username = "Username is required";
    else if (view === "signup" && /\s/.test(fields.username.value))
      errors.username = "Username cannot contain spaces";
  }

  // Email — signup and forgot only (not needed for signin)
  if (view !== "signin") {
  ...
  if (view !== "forgot") {
    ...
    } else if (view === "signup" && pw.length < 8) {
      // Length check only on signup — don't block signin
  ...
  if (view === "signup") {
    ...
    else if (confirm !== fields.password?.value) errors.confirm = "Passwords don't match";
  }
...
const AuthField: React.FC<{
  ...
  return (
    <TextField
      ...
      error={!!error} helperText={error} autoComplete={autoComplete}
      InputProps={{
        startAdornment: (
      ...
      sx={{
        "& .MuiOutlinedInput-root": {
          ...
          "&.Mui-focused": { bgcolor: isDark ? "rgba(255,255,255,0.06)" : "rgba(0,0,0,0.03)" },
          "&.Mui-focused .MuiOutlinedInput-notchedOutline": { borderColor: gold, borderWidth: "1.5px" },
        },
        "& .MuiInputLabel-root.Mui-focused": { color: gold },
        "& .MuiFormHelperText-root": { fontFamily: theme.typography.fontFamily, fontSize: "0.72rem" },
      }}
    />
  );
};
...
// ── OAuth button ──────────────────────────────────────────────────────────────
const OAuthButton: React.FC<{ icon: React.ReactNode; label: string; onClick?: () => void }> = ({
  icon, label, onClick,
}) => {
  const theme  = useTheme();
  const isDark = theme.palette.mode === "dark";
  return (
    <Button fullWidth variant="outlined" startIcon={icon} onClick={onClick}
      sx={{
        fontFamily: theme.typography.fontFamily, fontWeight: 600, fontSize: "0.85rem",
        textTransform: "none", borderRadius: 2, py: 1,
        borderColor: isDark ? "rgba(255,255,255,0.15)" : "rgba(0,0,0,0.15)",
        color: theme.palette.text.secondary,
        "&:hover": {
          borderColor: theme.palette.text.primary, color: theme.palette.text.primary,
...
const TabPill: React.FC<{ label: string; active: boolean; onClick: () => void }> = ({
  ...
  return (
    <Button onClick={onClick} disableRipple sx={{
      fontFamily: theme.typography.fontFamily, fontWeight: active ? 700 : 500,
      fontSize: "0.9rem", textTransform: "none",
      color: active ? gold : theme.palette.text.disabled,
      px: 0, pb: 1.2, borderRadius: 0,
      borderBottom: "2px solid", borderColor: active ? gold : "transparent",
      transition: "all 0.2s",
...
export const AuthModal: React.FC<AuthModalProps> = ({
  open, onClose, defaultView = "signin",
  onSuccess, loading: externalLoading = false, error: externalError = null,
}) => {
  const theme  = useTheme();
  const isDark = theme.palette.mode === "dark";
  const gold   = theme.palette.gold.main;
  ...
  const [username, setUsername] = useState<FieldState>(emptyField());
  ...
  // Sync view when parent changes defaultView
  useEffect(() => { setView(defaultView); }, [defaultView]);

  const switchView = (v: AuthView) => {
    setView(v);
    setLocalFeedback(null);
    setUsername(s => ({ ...s, error: "" }));
    setEmail   (s => ({ ...s, error: "" }));
  ...
  const handleClose = () => {
    ...
    setTimeout(() => {
      setView(defaultView);
      setUsername(emptyField()); setEmail(emptyField());
  ...
  const applyErrors = (errs: Record<string, string>) => {
    if (errs.username) setUsername(s => ({ ...s, error: errs.username }));
  ...
  const handleSubmit = () => {
    setLocalFeedback(null);
    const errs = validate(view, { username, email, password, confirm });

    if (view === "signup" && !agreed) { setAgreedErr(true); return; }
    if (Object.keys(errs).length > 0) { applyErrors(errs); return; }

    if (view === "forgot") {
      // No API endpoint yet — show local message
      setLocalFeedback(`Reset link sent to ${email.value}`);
      return;
    }
    ...
    // Do NOT close here — Navbar closes the modal only after the API succeeds
  };
  ...
  // externalError (from API) takes priority over local forgot-password success
  const activeFeedback: { type: "success" | "error"; msg: string } | null =
    externalError  ? { type: "error",   msg: externalError  } :
    localFeedback  ? { type: "success", msg: localFeedback  } :
    null;
  ...
  return (
    <Dialog
      ...
      TransitionComponent={Fade} transitionDuration={280}
      PaperProps={{
        sx: {
          borderRadius: 3, bgcolor: theme.palette.background.paper,
          backgroundImage: "none", border: `1px solid ${dividerColor}`, overflow: "hidden",
        },
      }}
    >
        ...
        <Box sx={{ px: { xs: 3, sm: 4 }, pt: 3.5, pb: 4 }}>
          ...
          <Box display="flex" alignItems="center" justifyContent="space-between" mb={3}>
            <Box display="flex" alignItems="center" gap={1.2}>
              ...
              <Box component="img" src={logo} alt="Daykin"
          ...
          {view === "forgot" && (
            <Box mb={2.5}>
              ...
              <Typography sx={{ fontFamily: SORA, fontSize: "0.85rem", color: theme.palette.text.secondary }}>
                We'll send a reset link to your email.
              </Typography>
            </Box>
          )}

          {/* Feedback alert — API errors + forgot success */}
          <Collapse in={!!activeFeedback}>
            <Alert severity={activeFeedback?.type ?? "info"}
          ...
          <Stack spacing={2}>
            ...
            {/* Username — signin and signup (not forgot) */}
            {view !== "forgot" && (
              <AuthField
                id="auth-username" label="Username"
                value={username.value} error={username.error}
                onChange={(v) => setUsername({ value: v, error: "" })}
            ...
            {view === "forgot" && (
              <AuthField
                id="auth-forgot-email" label="Email address" type="email"
                value={email.value} error={email.error}
                onChange={(v) => setEmail({ value: v, error: "" })}
                icon={<EmailOutlinedIcon fontSize="small" />}
                autoComplete="email"
          ...
          {view === "signin" && (
            <Box display="flex" alignItems="center" justifyContent="space-between" mt={1.5}>
              <FormControlLabel
                ...
                sx={{ m: 0 }}
              />
              <Link component="button" onClick={() => switchView("forgot")} underline="none"
                sx={{ fontFamily: SORA, fontSize: "0.8rem", fontWeight: 600,
                  color: gold, "&:hover": { opacity: 0.8 } }}>
                Forgot password?
              </Link>
            </Box>
          )}
          ...
          {view === "signup" && (
            <Box mt={1.5}>
              <FormControlLabel
                control={
                  <Checkbox size="small" checked={agreed}
                ...
                label={
                  <Typography sx={{ fontFamily: SORA, fontSize: "0.8rem",
                    ...
                    I agree to the{" "}
                    <Link href="#" underline="none" sx={{ color: gold, fontWeight: 600 }}>Terms of Service</Link>
                    {" "}&{" "}
                    <Link href="#" underline="none" sx={{ color: gold, fontWeight: 600 }}>Privacy Policy</Link>
                  </Typography>
                }
          ...
          <Button fullWidth variant="contained"
            ...
            sx={{
              ...
              textTransform: "none", bgcolor: gold, color: "#0D0D0D",
              "&:hover": { bgcolor: "#D4891A" },
              "&:disabled": {
                bgcolor: isDark ? "rgba(245,166,35,0.35)" : "rgba(245,166,35,0.4)",
                color: "#0D0D0D",
              },
            }}
          >
