...
interface Props { article: Article; onClose: () => void; audio: AudioState; }

export const ArticleModal: React.FC<Props> = ({ article, onClose, audio }) => {
  ...
  return (
    <div ref={overlayRef}
      ...
      style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.85)",
        zIndex: 300, overflowY: "auto", display: "flex",
      ...
      <div style={{ background: "#141414",
        ...
        <button onClick={onClose} style={{ position: "absolute",
          top: "1rem", right: "1.2rem", background: "none",
          border: "none", fontSize: "1.3rem", cursor: "pointer",
          color: "rgba(255,255,255,0.4)", lineHeight: 1,
          fontFamily: SERIF }}>✕</button>

        <div style={{ marginBottom: "1.25rem" }}>
          <CreatorHeader article={article} />
        </div>

        <span style={{ fontFamily: SERIF, fontSize: "0.72rem",
          letterSpacing: "0.06em", textTransform: "uppercase",
          color: article.categoryColor ?? "#F59E0B", display: "block",
          marginBottom: "1rem" }}>
          {article.category}
        </span>
        ...
        <p style={{ fontFamily: SERIF, fontSize: "0.85rem",
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "0.8rem",
          ...
          <button onClick={() => audio.toggle(article.id)}
          ...
          <div style={{ flex: 1 }}>
            <input type="range" min={0} max={100} step={0.5}
              ...
              style={{ width: "100%", accentColor: article.categoryColor ?? "#F59E0B",
                cursor: "pointer" }} />
          </div>
          <span style={{ fontFamily: SERIF, fontSize: "0.75rem",
        ...
        <div style={{ maxWidth: 640 }}>
          {/* ✅ Guard: body is optional — fall back to excerpt if not provided */}
          {(article.body && article.body.length > 0) ? (
            article.body.map((para, i) => (
              <p key={i} style={{ fontFamily: SERIF, fontSize: "1.05rem",
            ...
            <p style={{ fontFamily: SERIF, fontSize: "1.05rem",
              lineHeight: 1.68, color: "rgba(255,255,255,0.8)",
              marginBottom: "1.4em" }}>
              {article.excerpt}
            </p>
        ...
        <div style={{ maxWidth: 640 }}>
          <EngagementBar engagement={article.engagement} color={article.categoryColor ?? "#F59E0B"} />
        </div>
      </div>
    </div>
  );
